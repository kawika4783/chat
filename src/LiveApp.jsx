...
function LiveMessenger({ user, onLogout }) {
  ...
  return <main className="live-shell">
    <aside className="live-sidebar">
      ...
      {users.length ? <div className="live-user-results">{users.map(result => <button key={result.id} onClick={() => startConversation(result.id)}><LiveAvatar user={result} size="sm" /><span className="live-result-copy"><strong>{result.name}</strong><small>Halo member</small></span><MessageCircle size={17} /></button>)}</div> : null}
